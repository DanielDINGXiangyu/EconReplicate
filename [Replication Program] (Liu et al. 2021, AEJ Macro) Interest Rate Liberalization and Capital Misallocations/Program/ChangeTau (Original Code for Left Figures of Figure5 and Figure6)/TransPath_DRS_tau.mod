warning off all
% dynare code to compute the transitional path of interest rate liberalization
% decreasing return to scale with time varying tau
% 2018.10.9
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
var Rt rst Nt Hst Hpt Kst Kpt Kt Kest Kept Lst Lpt Sst Spt Nst Npt Ast Apt At Yt Ct Ypt Yst dDst dDpt
    ...
    phit Ut taut;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
varexo phi_sst tau_sst;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
parameters alpha del gamma phi0 phi1 tau tau0 tau1 theta b mu_s mu_p sig_s sig_p rho_phi beta z_s z_p alftil kk;
load ParaSet
...
est_s   = ((rst+del)/(alftil*taut*z_s^alftil*Rt))^(1/alftil)*Hst(-1)^((1-alftil)/alftil);
esmt_s  = ((rst+del+phit)/(alftil*taut*z_s^alftil*Rt))^(1/alftil)*Hst(-1)^((1-alftil)/alftil);
esst_s  = ((rst+del+phit)/(alftil*taut*z_s^alftil*Rt))^(1/alftil)*((1+b*theta)*Hst(-1))^((1-alftil)/alftil);
...
dDst  = 1-del+taut*Rt(+1)*z_s^alftil*Fun_ek(est_s(+1),kk*alftil)*alftil*Hst^(alftil-1)*(Ek_s-Ek_est_s(+1))
      -(del+rst(+1))*Fun_ek(est_s(+1),kk)*(Ek_s-Ek_est_s(+1))+(del+rst(+1))*F_est_s(+1)
      +taut*Rt(+1)*z_s^alftil*alftil*Hst^(alftil-1)*(Ea_est_s(+1)-Ea_esmt_s(+1))
      +taut*Rt(+1)*z_s^alftil*Fun_ek(esmt_s(+1),kk*alftil)*alftil*Hst^(alftil-1)*(Ek_esmt_s(+1)-Ek_esst_s(+1))
      -(del+rst(+1)+phit(+1))*Fun_ek(esmt_s(+1),kk)*(Ek_esmt_s(+1)-Ek_esst_s(+1))+(del+rst+phit(+1))*(F_esst_s(+1)-F_esmt_s(+1))
      +(1+b*theta)^alftil*taut*Rt(+1)*z_s^alftil*alftil*Hst^(alftil-1)*Ea_esst_s(+1)-(del+rst(+1)+phit(+1))*b*theta*(1-F_esst_s(+1));
...
Rt = (1-gamma)/(taut*Kest+Kept)^gamma;

Ypt = Kept^(1-gamma)*Npt^gamma;
Yst = Kest^(1-gamma)*Nst^gamma;

Npt =     Kept/(Kept+taut*Kest);
Nst = taut*Kest/(Kept+taut*Kest);

phit-phi_sst = rho_phi*(phit(-1)-phi_sst);
taut-tau_sst = 0*(taut(-1)-tau_sst);
...
phit=phi0; phi_sst=phi0; Ut=U_0;taut=tau0; tau_sst=tau0;
...
phit=phi1; phi_sst=phi1; Ut=U_1;taut=tau1; tau_sst=tau1;
end;
steady;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
